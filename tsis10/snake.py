...
len = 110
input_rect = pygame.Rect(230, 550, 200, 43)
active = False
text = ''
image = pygame.image.load('/Users/macbook/Desktop/pp2/tsis10/original.jpg')
scale = pygame.transform.scale(image, (image.get_width()  * 1.57 , image.get_height() ))
image1 = pygame.image.load('/Users/macbook/Desktop/pp2/tsis10/f.jpg')
...
class Food: # создаем класс Food
    def __init__(self): # функция для инициализации
        self.x = random.randint(35, 710) # центр выбирается рандомно
        self.y = random.randint(95, 550)
...
class Snake: # создаем класс Snake
    ...
    #                 pygame.draw.line(screen, BLACK, (780 - len, S1.a + 250), (780, S1.a + 250), 5)
        ...
        if self.direction == 'RIGHT': # если направление - "вправо" и змейка не выходит за границы экрана, то она двигается вправо (координаты элементов по Ох увеличиваются на 5), а в противном случае возвращается влево, на начальную координату по Ох
            ...
            if self.level == 1:
                if self.elements[0][1] <= 600 and self.elements[0][1] >= 96:
                    self.elements[0][1] += self.dy
                else:
                    if self.score >= 3:
                        self.game_over()
...
def aq():
    ...
    while not do:
        ...
        pygame.draw.rect(screen, BLACK, (20, 85, 760, 530), 3) # прямоугольник для выделения игрового поля
...
def new_user():
    ...
    while not did:
        ...
        screen.blit(new_user_text, (170, 560))
        screen.blit(welcome_text, (220, 520))
        screen.blit(text_surface, (400, 520))
        pygame.display.flip()
        time.sleep(3)
        aq()
    pygame.quit()
...
def old_user():
    ...
    while not will:
        ...
        screen.blit(scale, (0, 0))
        # screen.blit(new_user_text, (170, 530))
        screen.blit(welcome_back_text, (200, 530))
        screen.blit(text_surface, (450, 530))
        screen.blit(your_prev_score, (50, 580))
        screen.blit(previous_score_text, (340, 580))
        screen.blit(your_prev_level, (400, 580))
        screen.blit(previous_level_text, (680, 580))
        pygame.display.flip()
        time.sleep(5)
        aq()
    pygame.quit()
...
while not done:
    for event in pygame.event.get():
        ...
        screen.blit(enter_name_text, (120, 430))
        screen.blit(scale, (0, 0))
