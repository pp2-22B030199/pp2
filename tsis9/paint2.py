...
def main():
    ...
    points = list() # Создаем пустой лист для точек треугольников и ромба

    def Rect_pos(x1, y1, x2, y2):  # Функция для определения координат, длины и ширины прямоугольника
        return pygame.Rect(min(x1, x2), min(y1, y2), abs(x1 - x2), abs(y1 - y2)) 

    def Square_pos(x1, y1, x2, y2): # Функция для определения координат и длины квадрата
        return pygame.Rect(min(x1, x2), min(y1, y2), abs(x1 - x2), abs(x1 - x2))

    def Equilateral_Triangle_pos(x1, y1, x2, y2): # Функция для определения координат точек правильного треугольника
        points.append((x1, y1))
        points.append((x2, y2))
        points.append((x2+x2-x1, y1))
        return points

    def Right_Triangle_pos(x1, y1, x2, y2): # Функция для определения координат прямоугольного треугольника
        points.append((x1, y1))
        points.append((x2, y2))
        points.append((x1, y2))
        return points

    def Rhombus_pos(x1, y1, x2, y2): # Функция для определения координат ромба
        points.append((x1, y1))
        points.append((x2, y2))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN: # если нажали какую-то клавишу клавиатуры
                ...
                if event.key == pygame.K_c: # если нажали C, то фигура - круг
                    figure = 'circle'
                if event.key == pygame.K_s: # если нажали S, то фигура - квадрат
                    figure = 'square'
                if event.key == pygame.K_u: # если нажали E, то фигура - правильный треугольник
                    figure = 'equilateral triangle'
                if event.key == pygame.K_t: # если нажали T, то фигура - прямоугольный треугольник
                    figure = 'right triangle'
                if event.key == pygame.K_a: # если нажали A, то фигура - ромб
                    figure = 'rhombus'
